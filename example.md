...
[阶段 1/2] 准备阶段 (PREPARE)
------------------------------------------------------------
→ 发送PREPARE到 P1... ✓ VOTE_YES
→ 发送PREPARE到 P2... ✓ VOTE_YES
→ 发送PREPARE到 P3... ✓ VOTE_YES
...
[阶段 2/2] 中止阶段 (ABORT)
------------------------------------------------------------
→ 发送ABORT到 P1... ✓ ACK_ABORT
→ 发送ABORT到 P2... ✓ ACK_ABORT
→ 发送ABORT到 P3... ✓ ACK_ABORT
...
在协调者运行期间，启动新的参与者：
```bash
python participant.py P4 6004
```

使用 `list` 命令可以看到新参与者已加入。

### 场景B: 不同的事务数据

尝试不同格式的数据：
```
data> user=charlie,action=login,timestamp=2024-01-01
data> product=laptop,price=999,quantity=5
data> sensor=temp01,value=25.5,unit=celsius
```
